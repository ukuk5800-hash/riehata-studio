    ...
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center px-4">
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-10 max-w-md w-full text-center">
          ...
          <p className="text-slate-600 text-sm mb-6">確認メールをお送りしました。メールをご確認の上、ログインしてください。</p>
          <Link href="/auth/login" className="px-6 py-3 bg-teal-600 text-white rounded-xl font-medium hover:bg-teal-700 text-sm inline-block">
            ログインする
          </Link>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center px-4 py-8">
      <div className="w-full max-w-md">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-8">
          <h1 className="text-2xl font-bold text-slate-900 mb-1">新規会員登録</h1>
          <p className="text-slate-500 text-sm mb-6">メールアドレスだけで登録できます</p>

          {/* Google */}
          <button className="w-full flex items-center justify-center gap-3 py-3 border-2 border-slate-200 rounded-xl text-sm font-medium text-slate-700 hover:bg-slate-50 mb-4">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <p className="flex items-center gap-1 text-xs text-slate-400 mt-1">
                <Info size={12} />電話番号は必須ではありません。なくても登録できます。
              </p>
            </div>
            ...
            <label className="flex items-start gap-3 cursor-pointer">
              ...
              <span className="text-sm text-slate-600">
                ...
                <Link href="/privacy" className="text-teal-600 hover:underline">プライバシーポリシー</Link>に同意する
              </span>
            </label>

            <button type="submit" disabled={loading || !agreed}
              className="w-full py-3 bg-teal-600 text-white font-semibold rounded-xl hover:bg-teal-700 disabled:opacity-50 disabled:cursor-not-allowed text-sm">
          ...
          <p className="text-center text-sm text-slate-500 mt-6">
            すでにアカウントをお持ちの方は{' '}
            <Link href="/auth/login" className="text-teal-600 font-medium hover:underline">ログイン</Link>
          </p>
        </div>
      </div>
    </div>
